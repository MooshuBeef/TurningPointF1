...
######## deleting this for now cause I think it's messing up the subtraction
#driver_laps["LapTime(s)"] = driver_laps["LapTime"].dt.total_seconds()
...
percentile_lap_times = driver_laps.groupby('Driver')['LapTime'].quantile(percentile_value)
...
sorted_percentile_lap_times['LapTimeDelta'] = sorted_percentile_lap_times['LapTime'] - median_lap_time
...
with open('sorted_percentile_lap_times.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
    # write the data
    pd.DataFrame(sorted_percentile_lap_times_update).to_csv('sorted_percentile_lap_times.csv')
################
"""
